. $env:r\function.ps1
# nanazip https://github.com/M2Team/NanaZip/issues/86
$packages = 'XP8K0HKJFRXGCK', '9NFH4HJG2Z9H', '9NCBCSZSJRSB', '9NZVDKPMR9RD', 'XPDC2RH70K22MN', 'BlueStack.BlueStacks', '9PMZ94127M4G', 'Microsoft.VisualStudioCode', 'Python.Python.3', 'lycheeverse.lychee', 'XP99J3KP4XZ4VV', '9N3SQK8PDS8G'
...
# TODO: request ability to install multiple-venes with one command https://github.com/pypa/pipx/issues/88
...
foreach ($package in $debloat) { sudo winget uninstall -h $package }
...
# firefox user.js
if (!$env:vm) {
  $env:defaultProfile = (cat $env:APPDATA\Mozilla\Firefox\profiles.ini | Select-String -Pattern 'Default=1' -Context 1 | % { $_.Context.PreContext[0] } | Select-String '(Profiles).*').Matches.Value

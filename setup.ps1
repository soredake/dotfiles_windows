$machine = 'BlueStack.BlueStacks', 'lycheeverse.lychee', 'AppWork.JDownloader', 'Google.Drive', 'GOG.Galaxy', 'DupeGuru.DupeGuru', 'Syncplay.Syncplay', '7zip.7zip', 'alexx2000.DoubleCommander', 'AntibodySoftware.WizTree', 'Rclone.Rclone', 'yt-dlp.yt-dlp', 'Gyan.FFmpeg', 'Parsec.Parsec', 'LogMeIn.Hamachi', 'ViGEm.ViGEmBus', 'Microsoft.PowerToys'
$user = 'XP8K0HKJFRXGCK', '9NFH4HJG2Z9H', '9NZVDKPMR9RD', 'XPDC2RH70K22MN', '9PMZ94127M4G', 'Python.Python.3.11', 'XP99J3KP4XZ4VV', '9N3SQK8PDS8G', 'XPFM5P5KDWF0JP', 'SteamGridDB.RomManager', '64Gram.64Gram', 'Postman.Postman', 'ResponsivelyApp.ResponsivelyApp', 'RussellBanks.Komac', 'RamenSoftware.7+TaskbarTweaker', 'nomacs.nomacs' # nanazip https://github.com/M2Team/NanaZip/issues/86
# $user += '9N95Q1ZZPMH4', '9PG2DK419DRG', '9MVZQVXJBQ9V', '9NCTDW2W1BH8', '9PMMSR1CGPWG', '9N5TDP8VCMHS'
foreach ($p in $machine) { sudo winget install --no-upgrade -h --accept-package-agreements --accept-source-agreements $p }
...
sudo ~\Downloads\Sophia*\Sophia.ps1 -Function CreateRestorePoint, "TaskbarSearch -SearchIcon", "CastToDeviceContext -Hide", "ControlPanelView -LargeIcons", "FileTransferDialog -Detailed", "HiddenItems -Enable", "FileExtensions -Show", "TaskbarChat -Hide", "ShortcutsSuffix -Disable", "UnpinTaskbarShortcuts -Shortcuts Edge, Store", "OneDrive -Uninstall", "DNSoverHTTPS -Enable -PrimaryDNS 1.1.1.1 -SecondaryDNS 1.0.0.1" #, "HEVC -Install" # "ThumbnailCacheRemoval -Disable"
sudo choco install -y --pin ds4windows syncthingtray achievement-watcher itch protonvpn tor-browser # syncthingtray is pinned until https://gitlab.com/yan12125/chocolatey-packages/-/issues/2 is fixed
sudo choco install -y ryujinx virtualbox multipass ytdownloader taiga keepassxc insomnia nodejs.install ppsspp retroarch steascree.install choco-cleaner msiafterburner nerdfont-hack tor --params "/DesktopShortcut /NoShellHereIntegration /NoOpenSSH /RTSSDesktopShortcut /No32bitIcon /HyperVisor:VirtualBox"; sudo choco install -y --pre mpvnet.portable pcsx2-dev rpcs3 --params "/DesktopShortcut"; sudo choco install -y --pin --ignorechecksum ea-app; sudo choco install -y choco-upgrade-all-at --params "'/WEEKLY:yes /DAY:SUN /TIME:19:00'"
...
sudo "$env:TEMP/Winget-AutoUpdate-main/Winget-AutoUpdate-Install.ps1" -Silent -NotificationLevel None -UpdatesInterval Weekly -DoNotUpdate -UpdatesAtTime 11AM
...
sudo --ti reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Communications" /v "ConfigureChatAutoInstall" /t REG_DWORD /d 0 /f # TODO: add to to sophi-script
...
sudo powercfg /devicedisablewake "Intel(R) I211 Gigabit Network Connection" # TODO: report this to qbittorent @ test with non-store version TODO: https://github.com/PowerShell/PowerShell/issues/13183
# vbs disable script
...
foreach ($package in $debloat) { sudo winget uninstall --accept-source-agreements -h $package }
Remove-Item -Path "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Startup\Achievement Watcher.lnk" # https://github.com/Xav83/chocolatey-packages/pull/24
if (!$env:vm) {
